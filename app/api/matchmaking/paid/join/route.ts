...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[Matchmaking ${requestId}]     roomId: ${roomId}`)
    console.log(`[Matchmaking ${requestId}]   RESPONSE:`)
    ...
    console.log(`[Matchmaking ${requestId}]     roomId: ${payload.roomId} ${payload.roomId === roomId ? '✅' : '❌ MISMATCH!'}`)
    console.log(`[Matchmaking ${requestId}]     arenaId: ${payload.arenaId ?? 'null'}`)
    console.log(`[Matchmaking ${requestId}]     battleSeed: ${payload.battleSeed ?? 'null'}`)
    console.log(`[Matchmaking ${requestId}]     playerA: ${payload.playerA?.userId ? `${payload.playerA.userId} (${payload.playerA.trainerId})` : 'null'}`)
    console.log(`[Matchmaking ${requestId}]     playerB: ${payload.playerB?.userId ? `${payload.playerB.userId} (${payload.playerB.trainerId})` : 'null'}`)
    console.log(`[Matchmaking ${requestId}]     opponent: ${payload.opponent?.userId ?? 'null'}`)
    console.log(`[Matchmaking ${requestId}]     acks: A_match=${payload.acks?.playerAMatchAck} B_match=${payload.acks?.playerBMatchAck} A_arena=${payload.acks?.playerAArenaAck} B_arena=${payload.acks?.playerBArenaAck}`)
    console.log(`[Matchmaking ${requestId}]   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
    console.log(`[Matchmaking ${requestId}] FULL RESPONSE:`, JSON.stringify(payload, null, 2))
