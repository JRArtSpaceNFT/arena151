...
BEGIN
  SELECT id, username, current_trainer_id, current_team, current_locked_order, team_locked_at, balance, locked_balance
  ...
  SELECT id INTO v_existing_match_id FROM matches
  WHERE (player_a_id = p_user_id OR player_b_id = p_user_id)
    AND room_id = p_room_id AND game_mode = 'paid_pvp'
  ...
  FROM matches WHERE status = 'queueing' AND game_mode = 'paid_pvp' AND player_b_id IS NULL
    AND player_a_id != p_user_id AND room_id = p_room_id AND entry_fee_sol = p_entry_fee
    AND (expires_at IS NULL OR expires_at > now())
  ORDER BY created_at ASC FOR UPDATE SKIP LOCKED LIMIT 1;
  
  IF v_candidate_match_id IS NOT NULL THEN
    IF v_candidate_room_id != p_room_id THEN
      RAISE WARNING 'CROSS-ROOM LEAK DETECTED';
    ELSE
  ...
  INSERT INTO matches (
    id, player_a_id, player_a_username, player_a_trainer_id, player_b_id, entry_fee_sol,
    status, game_mode, room_id, team_a, player_a_locked_order, team_b, battle_seed, arena_id,
    idempotency_key, expires_at, created_at
  ) VALUES (
    v_new_match_id, p_user_id, v_profile.username, v_profile.current_trainer_id, NULL, p_entry_fee,
    'queueing', 'paid_pvp', p_room_id, v_profile.current_team, v_profile.current_locked_order, NULL,
    v_new_battle_seed, v_new_arena_id, gen_random_uuid()::text, now() + interval '5 minutes', now()
  );
