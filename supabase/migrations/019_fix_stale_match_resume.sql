-- Arena 151 — Fix Stale Match Resume Issue
-- Problem: atomic_join_or_create_paid_match resumes matches from days ago
-- Solution: Only resume matches created in the last 30 minutes
...
BEGIN
  ...
  WHERE (player_a_id = p_user_id OR player_b_id = p_user_id)
    ...
    AND created_at > now() - interval '30 minutes'  -- ← FIX: ignore stale matches
