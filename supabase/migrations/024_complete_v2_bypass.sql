-- Complete V2 bypass - full matchmaking without team lock requirement
...
BEGIN
  ...
  IF v_candidate_match_id IS NOT NULL THEN
    ...
    IF v_claimed_rows = 1 THEN
      ...
      VALUES (
        ...
        'wager_locked',
        p_entry_fee,
        jsonb_build_object('role', 'player_b', 'room_id', p_room_id, 'source', 'atomic_matchmaking_v2_bypass')
      );
  ...
  VALUES (
    ...
    'wager_locked',
    p_entry_fee,
    jsonb_build_object('role', 'player_a', 'room_id', p_room_id, 'source', 'atomic_matchmaking_v2_bypass')
  );
